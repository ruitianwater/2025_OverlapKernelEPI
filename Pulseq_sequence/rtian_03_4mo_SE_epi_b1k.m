...
% This demo script can produce 4-shot readout-phase-mosaic sequences,
...
% 4-shot, mosaic readout-phase-segmented, 2-slice, b=0,1000s/mm2, 2D navigator ON
...
Ny=150; % Define pixel size along phase encoding, here, for 1 shot in this 4-shot scheme
Nx=146; % Define pixel size along readout, here, for 1 shot in this 4-shot scheme
Nx_op=16; Nx = Nx+round(Nx_op/2); % overlapped pixel along readout, for 4-shot scheme
...
Ny_op=16; % Define the minimum required overlapped pixel number along phase encoding
Nshot = 4; % Number of shot, here, only 4
bSP = 0; % Here, keep bSP=0 (if switching polarity between bands)
bSegShift = 0; % Here, keep bSegShift=0 (if segment shift is enabled. 0 :disabled, 1: enabled)
nPreP = 2.5; % Here, keep 2.5. (multiplier for the pre phase duraction to the readout time)
...
if Nshot~=4
    error('This script only works for 4 shot phase-segmented EPI!') % rtian-2shot
end
Nx_tot = Nx*Nshot/2; % Nx*Nshot-(Nshot-1)*Ny_op; % rtian-2shot
Ny_tot = Ny*Nshot/2; % rtian-2shot
...
readoutTime=6.0e-4;     % this controls the readout bandwidth, standard 3.2e-4
partFourierFactor=8/8;   % partial Fourier factor: 1: full sampling
...
% phase encoding and partial Fourier,% rtian-2shot
...
Ny_pre = round((1/nPI)*partFourierFactor*Ny_op/2); % PE steps prior to ky=0, excluding the central line
Ny_meas = round((1/nPI)*Ny);
Ny_post = Ny_meas;
Ny_meas = Ny_meas+Ny_pre;
disp(['The additional overlap phase encoded line number:',num2str(Ny_pre)])
disp(['The total phase encoded line number:',num2str(Ny_meas)])
...
for idx_ds = 1:nDummy
    ...
    for s=[(1:2:Nslices),(2:2:Nslices)]
        ...
        % Diffusion gradient or just gradient echo delay
        seq.addBlock(mr.makeDelay(delayTE2));
  
        % Calculate prephase readout gradient scale factor
        ind_band = -(ms-1-(Nshot-1)/2);
        
        % rtian-4shot, gradient scaling factor
        switch ms
            case 1
                factor = round(Nx_op/2)/(Nx/2);
            case 2
                factor = round(Nx_op/2)/(Nx/2);
            case 3
                factor = -round(Nx_op/2)/(Nx/2);
        ...
        % adjust the y gradient polarity
        gyPre_temp = gyPre;
        if (ms == 1) || (ms == 3)
            gyPre_temp.amplitude=-gyPre_temp.amplitude;
            gyPre_temp.area=-gyPre_temp.area;
            gyPre_temp.flatArea=-gyPre_temp.flatArea;
        end

        [gxPre_temp,gyPre_temp]=mr.align('right',gxPre_temp,'right',gyPre_temp);
        seq.addBlock(gyPre_temp,gxPre_temp);

        % adjust the y gradient polarity
        gy_blipup_temp = gy_blipup;
        gy_blipdown_temp = gy_blipdown;
        gy_blipdownup_temp= gy_blipdownup;

        if (ms == 1) || (ms == 3) % rtian-4shot
            gy_blipup_temp.waveform=-gy_blipup_temp.waveform;
            gy_blipdown_temp.waveform=-gy_blipdown_temp.waveform;
            gy_blipdownup_temp.waveform=-gy_blipdownup_temp.waveform;
            gy_blipup_temp.area=-gy_blipup_temp.area;
            gy_blipdown_temp.area=-gy_blipdown_temp.area;
            gy_blipdownup_temp.area=-gy_blipdownup_temp.area;

            gy_blipup_temp.first = -gy_blipup_temp.first;
            gy_blipup_temp.last = -gy_blipup_temp.last;
            gy_blipdown_temp.first = -gy_blipdown_temp.first;
            gy_blipdown_temp.last = -gy_blipdown_temp.last;
            gy_blipdownup_temp.first = -gy_blipdownup_temp.first;
            gy_blipdownup_temp.last = -gy_blipdownup_temp.last;
        end
        ...
        for i=1:Ny_meas
            if i==1
                seq.addBlock(gx,gy_blipup_temp); % Read the first line of k-space with a single half-blip at the end
            elseif i==Ny_meas
                seq.addBlock(gx,gy_blipdown_temp); % Read the last line of k-space with a single half-blip at the beginning
            else
                seq.addBlock(gx,gy_blipdownup_temp); % Read an intermediate line of k-space with a half-blip at the beginning and a half-blip at the end
            end
        ...
        if b2DNav == 1
            ...
            % adjust phase prephase, rtian-2shot
            Nav_gyPre_temp = Nav_gyPre;
            if (ms == 1) || (ms == 3) % rtian-4shot
                Nav_gyPre_temp.amplitude = -Nav_gyPre_temp.amplitude;
                Nav_gyPre_temp.area = -Nav_gyPre_temp.area;
                Nav_gyPre_temp.flatArea = -Nav_gyPre_temp.flatArea;
            end

            % Run the phase prephase for 2D navigator
            seq.addBlock(Nav_gyPre_temp);

            % Run the delay for 2D navigator
            seq.addBlock(mr.makeDelay(delayTE3));

            % adjust phase prephase, rtian-2shot
            gnav_y_blipup_temp = gnav_y_blipup;
            gnav_y_blipdown_temp = gnav_y_blipdown;
            gnav_y_blipdownup_temp = gnav_y_blipdownup;
            if (ms == 1) || (ms == 3) % rtian-4shot
                gnav_y_blipup_temp.waveform = -gnav_y_blipup_temp.waveform;
                gnav_y_blipup_temp.area = -gnav_y_blipup_temp.area;

                gnav_y_blipdown_temp.waveform = -gnav_y_blipdown_temp.waveform;
                gnav_y_blipdown_temp.area = -gnav_y_blipdown_temp.area;

                gnav_y_blipdownup_temp.waveform = -gnav_y_blipdownup_temp.waveform;
                gnav_y_blipdownup_temp.area = -gnav_y_blipdownup_temp.area;

                gnav_y_blipup_temp.first = -gnav_y_blipup_temp.first;
                gnav_y_blipup_temp.last = -gnav_y_blipup_temp.last;
                gnav_y_blipdown_temp.first = -gnav_y_blipdown_temp.first;
                gnav_y_blipdown_temp.last = -gnav_y_blipdown_temp.last;
                gnav_y_blipdownup_temp.first = -gnav_y_blipdownup_temp.first;
                gnav_y_blipdownup_temp.last = -gnav_y_blipdownup_temp.last;
            end
            ...
            for i=1:Ny_nav_meas
                if i==1
                    seq.addBlock(gnav_x,gnav_y_blipup_temp); % Read the first line of k-space with a single half-blip at the end
                elseif i==Ny_nav_meas
                    seq.addBlock(gnav_x,gnav_y_blipdown_temp); % Read the last line of k-space with a single half-blip at the beginning
                else
                    seq.addBlock(gnav_x,gnav_y_blipdownup_temp); % Read an intermediate line of k-space with a half-blip at the beginning and a half-blip at the end
                end
    ...
    for idx_dd = 1:(ndd+1)
        ...
        for ms = 1:Nshot
            ...
            for s=[(1:2:Nslices),(2:2:Nslices)]
                ...
                % adjust the y gradient polarity, rtian-2shot, rtian-4shot
                gyPre_temp = gyPre;
                if (ms == 1) || (ms == 3)
                    gyPre_temp.amplitude=-gyPre_temp.amplitude;
                    gyPre_temp.area=-gyPre_temp.area;
                ...
                % adjust the y gradient polarity, rtian-2shot
                gy_blipup_temp = gy_blipup;
                gy_blipdown_temp = gy_blipdown;
                gy_blipdownup_temp= gy_blipdownup;
                ...
                for i=1:Ny_meas
                    ...
                    if i==1
                        seq.addBlock(gx,gy_blipup_temp,adc); % Read the first line of k-space with a single half-blip at the end
                    elseif i==Ny_meas
                        seq.addBlock(gx,gy_blipdown_temp,adc); % Read the last line of k-space with a single half-blip at the beginning
                    else
                        seq.addBlock(gx,gy_blipdownup_temp,adc); % Read an intermediate line of k-space with a half-blip at the beginning and a half-blip at the end
                    end
                    gx.amplitude = -gx.amplitude;   % Reverse polarity of read gradient
                ...
                if b2DNav == 1
                    ...
                    for i=1:Ny_nav_meas
                        if i==1
                            seq.addBlock(gnav_x,gnav_y_blipup_temp,Nav_adc); % Read the first line of k-space with a single half-blip at the end
                        elseif i==Ny_nav_meas
                            seq.addBlock(gnav_x,gnav_y_blipdown_temp,Nav_adc); % Read the last line of k-space with a single half-blip at the beginning
                        else
                            seq.addBlock(gnav_x,gnav_y_blipdownup_temp,Nav_adc); % Read an intermediate line of k-space with a half-blip at the beginning and a half-blip at the end
                        end
                        gnav_x.amplitude = -gnav_x.amplitude;   % Reverse polarity of read gradient
                        seq.addBlock(mr.makeLabel('INC','LIN', 1));
                    end
