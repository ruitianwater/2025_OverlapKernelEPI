...
% This demo script can reconstruct data acquired from the 2-shot phase-mosaic sequences,
% in ...\Pulseq_sequence\,
...
para.bSP = 0; % readout polarity switch for gridding, keep 0 for this phase segmented EPI.
para.coeff_kai = 0; % Kaiser filter coeff, short readout axis smoothing, not applied for 2D navigator in this script for now
...
switch para.nEPI
    case 223
        para.seq_file_path = 'rtian_02_2ps_SE_epi_b1k.seq'; % multi-shot EPI sequence file
        nProj = 228; % MID, 1D navigator scan for odd-even echo correction
        para.seq_proj_file_path = 'rtian_02_proj.seq'; % sequence file for 1D navigator scan
        para.ESP = 1.08; % ESP [ms], see printout of the Pulseq sequence
        para.ind_kc = 3; % index of the central k-space phase encoded lines, for distortion corr. in SE-EPI
...
if para.bNav==1
    ...
    dat_2DNav(:,:,:,:,:,:,:,:,:,:,[1]) = flip(dat_2DNav(:,:,:,:,:,:,:,:,:,:,[1]),3); % for 4s-EPI, the 2D navigators have up-down directions flip
end
...
%% Load noise scan for prewhitening
...
% EPI gridding
[dat_epi_rg,P1,P2,para.mapS] = grid2DEPI(dat_epi,k_acq,para.nOS,para.nPI,1,para); % rtian-2shot
...
% k-space coverage, readout dim.: 2-shot phase segmented, only consider 1st band,
% without summing up since they are identical
...
ref_MTF_PE = zeros(length(P1),2);
loc_pe_b1 = find(P2==1);
ref_MTF_PE(1:loc_pe_b1(end),1) = 1;
loc_pe_b2 = find(P1==1);
ref_MTF_PE(loc_pe_b2(1):end,2) = 1;
...
if para.nPI>=1
    ...
    % rtian-2shot: Load the GRE images for estimating B0 maps
    if para.bDisCorr==1
    ...
    % If distortion correction (naive)
    if para.bDisCorr==1
    ...
    % SENSE reconstruction of each shot image
    parfor idx_RO = 1:ny
        for idx_3 = 1:n3_temp
            for idx_5 = 1:n5_temp
                for idx_6 = 1:n6_temp
    ...
    img_sense = img_sense.*reshape(ref_MTF_PE,1,[],2);
...
if (para.NSeg>1)  && (para.bIntSCorr==1)
        ...
        for idx_sl = 1:para.NSli
            for idx_cyc = 1:para.NPhs
                for idx_band = 1:(para.NSeg-1)
                    ...
                    % dat_mb: [Read,Phase,Seg,RF,2D Slice,NPhs(diff),NPar,NAve,NEco,NRep,NSet]
                    if para.nPFT ==1
                        dat_ol{idx_sl,idx_cyc,idx_band,idx_rep} = dat_mb(loc_ol{idx_band},ref_PE_MTF==1,[idx_band,idx_band+1],:,idx_sl,idx_cyc,:,:,:,idx_rep,:);
                    else
    ...
    % Perform ESPIRiT phase extraction, rtian-2shot
    ...
    % If directing summing up shot images, without using another forward
    % model for shot combine
    if para.bMolComb ==0 % rtian-2shot
    ...
    if para.NSeg>1
        if para.bIntSCorr ==1
            ...
            if para.coeff_kai ~=0
                ...
                ref_MTF_PE_sum = sum(ref_MTF_PE,2);
                ref_MTF_PE_sum(ref_MTF_PE_sum<1e-4)=1;
                dat_epi_comb= dat_epi_comb./reshape(ref_MTF_PE_sum,1,[]);
            end
        else
            ref_MTF_PE_sum = sum(ref_MTF_PE,2);
            ref_MTF_PE_sum(ref_MTF_PE_sum<1e-4)=1;
...
elseif para.bMolComb ==1
    ...
    if (para.NSeg>1)  && (para.bIntSCorr==1) % Self-navigated maps
        phase_map_temp = phase_map;
    elseif (para.bNav == 1) % 2D navigator maps
        phase_map_temp = img_Nav./(abs(img_Nav)+1e-10);
        if para.bConjNav ==1
            phase_map_temp = conj(phase_map_temp);
        end
    else % No inter-shot corrections
        phase_map_temp=1;
    end

    % Forward model combine
    parfor idx_5 = 1:n5_temp
        for idx_6 = 1:n6_temp
            for idx_7 = 1:n7_temp
                for idx_8 = 1:n8_temp
                    for idx_9 = 1:n9_temp
                        for idx_10 = 1:n10_temp
                            for idx_11 = 1:n11_temp
                                ...
                                Afun_reco_2PEPI = @(x,flag) reco_2PEPI(x,flag,phase_temp,...
                                    ny,nx,n3_temp,ref_MTF_shots,ref_MTF_PE);

                                [x_temp,flag,relres,iter,resvec,lsvec] = lsqr(Afun_reco_2PEPI,dat_temp,[],para.iterLSQR_combShot);
...
%% Function to combine 2-shot phase segments

function y = reco_2PEPI(x,flag,phase_temp,...
    ny,nx,nband,ref_MTF_shots,ref_MTF_PE)
if strcmp(flag,'notransp') % Compute A*x
    ...
    y = y.*reshape(ref_MTF_PE,1,[],size(ref_MTF_PE,2));
...
elseif strcmp(flag,'transp') % Compute A'*x
    ...
    x = x.*reshape(ref_MTF_PE,1,[],size(ref_MTF_PE,2));
    ...
    % remove MTF
    y = SymIfft(y,[1 2]);
    ref_MTF_PE_temp = sum(ref_MTF_PE,2);ref_MTF_PE_temp(ref_MTF_PE_temp<1e-4)=1e6;
    y = y./reshape(ref_MTF_PE_temp,1,[],1);
    y = SymFft(y,[1 2]);
...
if nPI~=0
    % Estimate phase encoding full grid number
    kx_Step = abs(k_acq(2,1,round(size(k_acq,3)/2),1,1,1,1,1,1,1,1)-k_acq(2,1,round(size(k_acq,3)/2)-1,1,1,1,1,1,1,1,1))/nPI;
    para.nx_tot = length((kxmin:kx_Step:kxmax));

    if para.nx_tot<size(k_acq,3)
        para.nx_tot = size(k_acq,3);
    end

    kxx = kxmin:kx_Step:kxmax;
    kx_exp = kxx(1:nPI:end);

    % Calculate the sampling pattern, band from k-space center to positive
    % k-values
    kx_exp_1 = kx_exp(end:-1:(end-size(k_acq,3)+1)); % actual k-space sampling locations
    kx_exp_1 = flip(kx_exp_1,2);
    P1 = ismember(kxx, kx_exp_1); % The sampling index

    % Calculate the sampling pattern, band from k-space center to negative
    % k-values
    kx_exp_2 = kx_exp(1:size(k_acq,3)); % actual k-space sampling locations
    P2 = ismember(kxx, kx_exp_2); % The sampling index
else
    P = [];
    para.nx_tot = length(P);
end
...
cali_filters = cali_filters*V_filter; % maybe redundant together with another operation V_filter* later
cali_filters = reshape(cali_filters,[nky_subspace nkx_subspace id_trun nb]);
cali_filters = permute(cali_filters,[1,2,4,3]);
...
for id_y = 1:ny
    for id_x =  1:nx
        ...
        U_img = V_filter*(U_img.*ph_comp); % V_filter* maybe redundant with a previous step *V_filter
        S_img = diag(S_img);
...
% Naively summing the results from two directions of interpolations
coeff_2 = 1./coeff_2;
