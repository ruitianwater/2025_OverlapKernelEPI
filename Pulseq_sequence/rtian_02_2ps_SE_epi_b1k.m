...
% This demo script can produce 2-shot phase-mosaic sequences,
...
% 2-shot, mosaic phase-segmented, 2-slice, b=0,1000s/mm2, 2D navigator ON
...
Ny=112; % Define pixel size along phase encoding, here for 1 shot in this 2-shot scheme
Nx=220; % Define pixel size along readout
...
Ny_op=12; % Define the minimum required overlapped pixel number along readout
Nshot = 2; % Number of shot, here, only 2
...
nPreP = 1; % Here, keep 1. multiplier for the pre phase duraction to the readout time
...
if Nshot~=2
    error('This script only works for 2 shot phase-segmented EPI!') % rtian-2shot
end
Nx_tot = Nx; % Nx*Nshot-(Nshot-1)*Ny_op; % rtian-2shot
Ny_tot = Ny*Nshot; % rtian-2shot
...
readoutTime=9.8e-4;     % this controls the readout bandwidth, standard 3.2e-4
...
for idx_ds = 1:nDummy
    ...
    for s=[(1:2:Nslices),(2:2:Nslices)]
        ...
        if (bSP==1) && (mod(ind_band,2)==1)
            ...
            if bSegShift==1
                area_temp = -gx.area*(1/2+ind_band-Ny_op/Nx*ind_band-1);
            else
            ...
            if bSegShift==1
                area_temp = -gx.area*(1/2+ind_band-Ny_op/Nx*ind_band);
            else
                area_temp = -gx.area*(1/2);
            end
            factor = area_temp/(-gx.area*(1/2+nsb));
