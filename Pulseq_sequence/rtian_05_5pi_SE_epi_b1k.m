...
% This demo script can produce 5-shot phase-interleaved sequences,
...
Ny = 310; % Define pixel size along phase encoding
Nx = 310; % Define pixel size along readout
...
Nshot = 5; % Number of shot, should be odd
bSP = 0; % if switching polarity between bands
bSegShift = 1; % if segment shift is enabled. 0 :disabled, 1: enabled
nPreP = 1.0; % multiplier for the pre phase duraction to the readout time
bETS = 1.0; % Here, keep 1. Echo time shifting. 1: Yes, 0: No.
spoilFactor=1.5; % spoiling gradient around the pi-pulse 0: disabled crusher
...
nPI = 1; % parallel imaging acceleration factor along phase encoding dimension
...
readoutTime=9.6e-4; % this controls the readout bandwidth, 10.2e-4
...
disp(['High-resolution readout bandwidth = ', num2str(readoutBW), ' Hz/Px']) ;
...
blip_dur = ceil(2*sqrt(nPI*Nshot*deltak/lims.maxSlew)/10e-6/2)*10e-6*2; % we round-up the duration to 2x the gradient raster time
% the split code below fails if this really makes a trpezoid instead of a triangle...
gy = mr.makeTrapezoid('y',lims,'Area',-deltak*nPI*Nshot,'Duration',blip_dur); % we use negative blips to save one k-space line on our way towards the k-space center
%gy = mr.makeTrapezoid('y',lims,'amplitude',deltak/blip_dur*2,'riseTime',blip_dur/2, 'flatTime', 0);

% Phase blip for 2D navigator
blip_nav_dur = ceil(2*sqrt(nPI*deltak/lims.maxSlew)/10e-6/2)*10e-6*2; % we round-up the duration to 2x the gradient raster time
gy_nav = mr.makeTrapezoid('y',lims,'Area',-deltak*nPI,'Duration',blip_nav_dur);

% Phase blip for density variable sampling
nPI_ref = 3; % hardcode undersampling factor for commonly sampled region
blip_vd_dur = ceil(2*sqrt(nPI_ref*deltak/lims.maxSlew)/10e-6/2)*10e-6*2; % we round-up the duration to 2x the gradient raster time
gy_vd = mr.makeTrapezoid('y',lims,'Area',-deltak*nPI_ref,'Duration',blip_vd_dur); % we use negative blips to save one k-space line on our way towards the k-space center
...
disp(['effective echo spacing = ', num2str(ESP/nPI/Nshot), ' ms']) ;
...
gy_vd_parts = mr.splitGradientAt(gy_vd, blip_vd_dur/2, lims);
[gy_vd_blipup, gy_vd_blipdown,~]=mr.align('right',gy_vd_parts(1),'left',gy_vd_parts(2),gx);
gy_vd_blipdownup = mr.addGradients({gy_vd_blipdown, gy_vd_blipup}, lims);

gy_blipdownup_f2l = mr.addGradients({gy_blipdown, gy_vd_blipup}, lims);
gy_blipdownup_l2f = mr.addGradients({gy_vd_blipdown, gy_blipup}, lims);

% Navigator: split the blip
gy_nav_parts = mr.splitGradientAt(gy_nav, blip_nav_dur/2, lims);
[gnav_y_blipup, gnav_y_blipdown,~]=mr.align('right',gy_nav_parts(1),'left',gy_nav_parts(2),gnav_x);
...
gy_vd_blipup.waveform=gy_vd_blipup.waveform*pe_enable;
gy_vd_blipdown.waveform=gy_vd_blipdown.waveform*pe_enable;
gy_vd_blipdownup.waveform=gy_vd_blipdownup.waveform*pe_enable;

% phase encoding and partial Fourier
Ny_pre=round((1/nPI/Nshot)*partFourierFactor*Ny/2-1); % PE steps prior to ky=0, excluding the central line
Ny_post=round((1/nPI/Nshot)*Ny/2+1); % PE lines after the k-space center including the central line
...
Ny_diff = (Ny_post-1)*nPI*Nshot-Ny_nav_pre*nPI;
disp(['The central k-space line for 2D nav is at #',num2str(Ny_nav_pre+1), ' phase encoded lines'])

% Pre-phasing gradients
nsb = (Nshot-1)/2;
gxPre = mr.makeTrapezoid('x',lims_prephase,'Area',-gx.area*(1/2),'duration',ceil(nPreP*readoutTime./lims.gradRasterTime)*lims.gradRasterTime);
gyPre = mr.makeTrapezoid('y',lims_prephase,'Area',Ny_pre*deltak*nPI*Nshot,'duration',ceil(nPreP*readoutTime./lims.gradRasterTime).*lims.gradRasterTime);
...
% Nav_gyPre = mr.makeTrapezoid('y',lims,'Area',-Ny_diff*deltak*nPI*Nshot,'duration',nPreP*readoutTime*Nav_Nx/Nx);
Nav_gyPre = mr.makeTrapezoid('y',lims_prephase,'Area',-Ny_diff*deltak,'duration',ceil((nPreP*readoutTime*Ny_diff/(Ny_pre*nPI*Nshot))/lims.gradRasterTime)*lims.gradRasterTime);
...
delayTE3 = ceil((CenterToEnd + mr.calcDuration(gx) + Nav_rf180centerInclDelay - (mr.calcDuration(Nav_rf180,Nav_gz180n)-Nav_rf180centerInclDelay) - mr.calcDuration(Nav_gyPre) - tBuffTrain - Nav_durationToCenter)/lims.gradRasterTime)*lims.gradRasterTime;
TE2_nav = TE+CenterToEnd+mr.calcDuration(gx)+mr.calcDuration(Nav_rf180,Nav_gz180n)+ mr.calcDuration(Nav_gyPre)+ delayTE3+ tBuffTrain + Nav_durationToCenter;
...
if bTypeEcho == 1
    ...
    gr=ceil(lims.maxGrad/lims.maxSlew/lims.gradRasterTime)*lims.gradRasterTime;% rtian
    ...
    gDiff_x=mr.makeTrapezoid('x','amplitude',g,'riseTime',gr,'flatTime',small_delta-gr,'system',lims);
    gDiff_y=mr.makeTrapezoid('y','amplitude',g,'riseTime',gr,'flatTime',small_delta-gr,'system',lims);
    gDiff_z=mr.makeTrapezoid('z','amplitude',g,'riseTime',gr,'flatTime',small_delta-gr,'system',lims);
...
factor_sp = 30;
dscale_3T = 0.5;
gy_spoiler = mr.makeTrapezoid('y',lims_prephase,'Area',-factor_sp*Ny_post*deltak*nPI,'duration',nPreP*readoutTime/2*factor_sp*dscale_3T);
gz_spoiler = mr.makeTrapezoid('z',lims_prephase,'Area',factor_sp*Ny_post*deltak*nPI,'duration',nPreP*readoutTime/2*factor_sp*dscale_3T);
...
for idx_ds = 1:nDummy
    ...
    for s=[(1:2:Nslices),(2:2:Nslices)]
        ...
        % calculate phase blip in each shot
        gyPre_temp = gyPre;
        if mode_proj~=1
            factor = gyPre_temp.area - gy.area/Nshot*(ms-1);
            factor = factor./gyPre_temp.area;
            
            [gyPre_temp,time_reduced] = scaleGrad(lims,gyPre_temp,factor);
            [gxPre,gyPre_temp]=mr.align('right',gxPre,'left',gyPre_temp);
        else
            time_reduced = 0;
        end
        
        % Compensate the reduced time if exists
        seq.addBlock(gxPre,gyPre_temp);
        if (mr.calcDuration(gyPre_temp)+time_reduced)>mr.calcDuration(gxPre)
            seq.addBlock(mr.makeDelay(mr.calcDuration(gyPre_temp)+time_reduced-mr.calcDuration(gxPre)));
        end
        ...
        % Echo time shifting technique
        if bETS==1
            if Nshot~=ms
                seq.addBlock(mr.makeDelay(ceil(mr.calcDuration(gx)/Nshot*(Nshot-ms)/lims.gradRasterTime)*lims.gradRasterTime));
            end
        end
        ...
        if mod(Ny_meas,2)==1 % If odd PE lines, flip the gx to default
            gx.amplitude = -gx.amplitude;   % Reverse polarity of read gradient
        end
        ...
        if b2DNav == 1
            ...
            % readout blip for navigator
            gNav_xPre_temp = gx;% use gx instead of gNav_xPre_temp with 2x amplitude scales, which may boost PNS
            oddness_phase = mod(Ny_meas,2);
            
            if oddness_phase==0
                % [gNav_xPre_temp,time_reduced] = scaleGrad(lims,gNav_xPre_temp,(-2));
            else
                gNav_xPre_temp.amplitude = 0;
            end
            seq.addBlock(gNav_xPre_temp);

            % phase blip for navigator
            Nav_gyPre_temp = Nav_gyPre;
            
            scale_nav = (Nav_gyPre_temp.area+gyPre.area*(factor-1))/Nav_gyPre_temp.area;
            [Nav_gyPre_temp,time_reduced] = scaleGrad(lims,Nav_gyPre_temp,scale_nav);
            ...
            seq.addBlock(Nav_gyPre_temp);
            if time_reduced>0
                seq.addBlock(mr.makeDelay(time_reduced));
            end
            if delayTE3>0
        ...
        if bETS==1
            seq.addBlock(mr.makeDelay(round(ts_idle-ceil(mr.calcDuration(gx)/Nshot*(Nshot-ms)/lims.gradRasterTime)*lims.gradRasterTime,5)));
        else
            seq.addBlock(mr.makeDelay(round(ts_idle,5)));
        end
...
for idx_avg = 1:nRep
    ...
    for idx_dd = 1:(ndd+1)
        ...
        for ms = 1:Nshot
            ...
            seq.addBlock(mr.makeLabel('SET', 'SET', 0) ) ;
            for s=[(1:2:Nslices),(2:2:Nslices)]
                
                % RF excitation
                seq.addBlock(rf_fs,gz_fs,gx_fs,gy_fs);
                rf.freqOffset=gz.amplitude*thickness*(-dis_sl)*(s-1-(Nslices-1)/2);
                rf.phaseOffset=pi/2-2*pi*rf.freqOffset*mr.calcRfCenter(rf); % compensate for the slice-offset induced phase
                ...
                if ((nDummy==0) && (s==1) && (ms ==1) && (idx_dd==1))&& (idx_avg==1)
                    ts_block = sum(seq.blockDurations);
                    ts_idle = TR/Nslices-ts_block;
                    if ts_idle<0
                        error('too many slices')
                ...
                seq.addBlock(mr.makeLabel('INC','SET', 1)) ;
            end
...
if abs(scale)>1 % Do not extend the duration
